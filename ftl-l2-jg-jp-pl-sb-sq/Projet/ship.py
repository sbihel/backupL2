...
class Ship(object):
    ...
    def __init__(self, systems, listWeapons, rooms, nameShip, typeShip, listDrones, listAugmentation, ID, maxPower=10,
                 life=30, missiles=10, droneParts=6):
        """
        ...
        for k in self.__rooms:
            ...
            for d in k.getDoors():
                if -1 not in d.getLink():
                    otherRoom = [r2 for r2 in d.getLink() if r2 != k.getNB()][0]
    ...
    def refillOxygen(self):
        try:
            ...
            print('\033[31m'+" engines "+str(self.getSystem('oxygen').getPowerInIt())+' '+str(self.getSystem('oxygen').getCurrentMaxPower())+' '+str(self.getSystem('oxygen').getMaxPower())+'\033[0m')
        ...
        for k in range(len(self.__weaponControl.getWeapons())):
            if k is not None:
                ...
                if use[0]:
                    ...
                    if printTK:
                        displayShipTK(self, self.__weaponControl.getWeapons()[k],None, None)
                        displayShipTK(ship2, None, None, use[1])
    ...
    def attackUsingDrone(self, d, ship2):
        ...
        if use[0] and showShipState:
            ...
            if printTK:
                        displayShipTK(self, None, self.__droneControl.getDrones()[d], None)
                        displayShipTK(ship2, None, None, use[1])
