...
"""
Extracts the informations from the xml file to make a basic ship.
"""
...
def genShip(ID=1, nameShip='kestrel', typeShip='typeA'):
    ...
    listAugmentations = []
    for k in node.findall('augmentations')[0]:
        listAugmentations += [k.text]
    ...
    return Ship(systems, listWeapons, rooms, nameShip, typeShip, listDrones, listAugmentations, ID, energy)
