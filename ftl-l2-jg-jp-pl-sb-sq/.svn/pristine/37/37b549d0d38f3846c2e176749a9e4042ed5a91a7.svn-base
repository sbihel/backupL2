...
class CrewMember(object):
    ...
    def __eq__(self, other):
        return self.__position == other.getPosition() and self.__coordsGoal == other.getCoordsGoal()
    ...
    def goToRoom(self, nb):
        ...
        if numberPersonInOtherRoom < otherRoom.getNbBoxes():
            ...
            if otherRoom.getNbBoxes() == 4 and \
                    (otherRoom.getSystem() == 'medbay' and self.__ship.hasSystem('medbay') or
                     otherRoom.getSystem() == 'cloneBay' and self.__ship.hasSystem('cloneBay')):
    ...
    def doDamages(self, nb):
        self.__hp -= nb
        if self.isDead():
            if self.__ship.hasSystem('cloneBay'):
    ...
    def addXpPiloting(self):
        ...
        if self.__lvlPiloting < 2:
            self.__lvlPiloting += 0.01
            if 1 > self.__lvlPiloting >= 0.15*advantageRace[self.__race] \
    ...
    def addXpEngines(self):
        advantageRace = {'human': 0.9, 'engi': 1, 'mantis': 1, 'rockman': 1, 'zoltan': 1,
                         'slug': 1, 'crystal': 1, 'lanius': 1}
        if self.__lvlEngines < 2:
            self.__lvlEngines += 0.01
            if 1 > self.__lvlEngines >= 0.15*advantageRace[self.__race] \
    ...
    def addXpShields(self):
        advantageRace = {'human': 0.9, 'engi': 1, 'mantis': 1, 'rockman': 1, 'zoltan': 1,
                         'slug': 1, 'crystal': 1, 'lanius': 1}
        if self.__lvlShields < 2:
            self.__lvlShields += 0.01
            if 1 > self.__lvlShields >= 0.55*advantageRace[self.__race] \
    ...
    def addXpWeaponControl(self):
        advantageRace = {'human': 0.9, 'engi': 1, 'mantis': 1, 'rockman': 1, 'zoltan': 1,
                         'slug': 1, 'crystal': 1, 'lanius': 1}
        if self.__lvlWeaponControl < 2:
            self.__lvlWeaponControl += 0.01
            if 1 > self.__lvlWeaponControl >= 0.65*advantageRace[self.__race] \
    ...
    def addXpRepair(self):
        advantageRace = {'human': 0.9, 'engi': 1, 'mantis': 1, 'rockman': 1, 'zoltan': 1,
                         'slug': 1, 'crystal': 1, 'lanius': 1}
        if self.__lvlRepair < 2:
            self.__lvlRepair += 0.01
            if 1 > self.__lvlRepair >= 0.18*advantageRace[self.__race] \
    ...
    def addXpCombat(self):
        advantageRace = {'human': 0.9, 'engi': 1, 'mantis': 1, 'rockman': 1, 'zoltan': 1,
                         'slug': 1, 'crystal': 1, 'lanius': 1}
        if self.__lvlCombat < 2:
            self.__lvlCombat += 0.01
            if 1 > self.__lvlCombat >= 0.08*advantageRace[self.__race] \
